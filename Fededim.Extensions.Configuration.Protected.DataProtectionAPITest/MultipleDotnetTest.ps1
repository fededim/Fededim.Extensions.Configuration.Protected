...
for ($i = 1; $i -le $Iterations; $i++) {
	...
	dotnet test -c "$Configuration" --framework net4.8 --results-directory "$OutputDirectory" -- --report-trx --report-trx-filename "{tfm}-{arch}\testrun_$i.trx" --report-html --report-html-filename "{tfm}-{arch}\testrun_$i.html"
